...
export default function LandingPage() {
  ...
  return (
    <main className="h-screen w-full overflow-hidden bg-[#050505] text-white relative font-sans flex flex-col selection:bg-white/20">
      ...
      <nav className="relative z-50 w-full flex items-center justify-between px-6 sm:px-8 py-4 sm:py-5 border-b border-white/5 bg-[#050505]/50 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          <Image src="/logo.png" alt="DayFlow Logo" width={38} height={38} className="rounded-lg shadow-[0_0_20px_rgba(var(--color-brand-rgb),0.35)] md:w-[42px] md:h-[42px]" unoptimized />
          <span className="text-lg sm:text-xl font-black tracking-tight text-white">DayFlow</span>
        </div>
        <div>
          {session?.user ? (
            <Link href="/analytics" className="relative group active:scale-95 transition-transform block">
              <img
                ...
                alt="Profile"
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-white/10 group-hover:border-white/40 transition-colors shadow-lg"
                referrerPolicy="no-referrer"
              />
            </Link>
          ) : (
            <Link href="/login">
              <button className="text-xs sm:text-sm font-semibold text-white/70 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full transition-all">
                Sign In
              </button>
            </Link>
          )}
