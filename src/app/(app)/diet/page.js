...
function WaterTracker({ glasses, setGlasses, target = 8 }) {
  return (
    <Card className="p-8 border-b-4 border-b-[var(--color-work)]">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Droplet className="w-5 h-5 text-[var(--color-work)]" /> Water Intake
          </h2>
          ...
          {[...Array(target)].map((_, i) => (
            <motion.button
              ...
              className={`w-10 h-14 rounded-b-xl rounded-t flex items-end justify-center overflow-hidden border-2 transition-colors duration-300 ${
                i < glasses 
                  ? "border-[var(--color-work)] bg-[var(--color-work)]/10" 
            ...
            >
              <motion.div
                ...
                className="w-full bg-[var(--color-work)]"
  ...
  return (
    <PageWrapper className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setActiveModal("meal")} className="gap-2 bg-[var(--color-warning)] hover:bg-[#b04a28]">
          ...
          <div className="space-y-3">
            {meals.map(meal => (
              <motion.div key={meal.id} whileHover={{ scale: 1.01 }}>
                <Card className="flex items-center justify-between p-4 border-l-4 border-l-[var(--color-warning)]">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-[var(--color-warning)]/10 text-[var(--color-warning)] flex items-center justify-center">
                  ...
                  {meal.isHealthy ? (
                    <span className="flex items-center gap-1 text-xs font-medium text-[var(--color-success)] bg-[var(--color-success)]/10 px-2 py-1 rounded-full">
                      <CheckCircle2 className="w-3 h-3" /> Healthy
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 text-xs font-medium text-[var(--color-warning)] bg-[var(--color-warning)]/10 px-2 py-1 rounded-full">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-medium flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 text-[var(--color-warning)]" /> Junk Food
                </h4>
                <p className="text-xs text-slate-500 mt-1">Did you eat junk food today?</p>
              </div>
              <button 
                onClick={() => setJunkFood(!junkFood)}
                className={`w-12 h-6 rounded-full p-1 transition-colors ${junkFood ? "bg-[var(--color-warning)]" : "bg-slate-200 dark:bg-slate-700"}`}
              >
                <motion.div 
