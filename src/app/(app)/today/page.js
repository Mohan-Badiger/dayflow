  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }} className="h-full">
      <Link href={href} className="card h-full p-4 md:p-5 flex flex-col justify-between group hover:border-border-2 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 bg-surface-1/60 backdrop-blur-md relative overflow-hidden border border-border/50">
        <div className="absolute top-0 right-0 w-32 h-32 rounded-full opacity-10 blur-3xl transition-opacity group-hover:opacity-30" style={{ backgroundColor: color }} />
        
        <div className="flex items-center justify-between relative z-10 mb-6">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center shrink-0 transition-transform duration-300 group-hover:scale-110 shadow-inner" style={{ backgroundColor: color + "15", color: color, boxShadow: `inset 0 0 0 1px ${color}30` }}>
        ...
        <div className="relative z-10 mt-auto">
          ...
          <p className="text-[10px] md:text-[11px] text-text-3 mt-1.5 font-medium min-h-[16px] flex items-center">{sublabel || " "}</p>
        </div>
      </Link>
    </motion.div>
  );
...
export default function TodayPage() {
  ...
  return (
    <PageWrapper className="space-y-6 pb-8">
      ...
      {/* ─── Score + Overview ────────────────────── */}
      <div className="card p-8 relative overflow-hidden bg-surface-1/50 backdrop-blur-xl border-border-2/50 shadow-2xl">
            ...
            {timetable.slice(0, 6).map((block, i) => {
              ...
              return (
                <motion.div key={block._id || i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  className={`shrink-0 w-40 p-4 rounded-2xl border backdrop-blur-sm transition-all shadow-xs ${isDone ? "opacity-50" : "hover:shadow-md hover:-translate-y-0.5"}`}
      ...
      {/* ─── Study Quick Start ───────────────────── */}
      <Link href="/sessions" className="group relative overflow-hidden rounded-2xl p-6 flex items-center gap-5 border border-brand/20 bg-linear-to-r from-brand/10 to-brand/5 shadow-lg hover:shadow-brand/20 transition-all duration-300 hover:-translate-y-1">
        <div className="absolute -right-6 -top-6 w-32 h-32 bg-brand/10 rounded-full blur-2xl group-hover:bg-brand/20 transition-colors" />
