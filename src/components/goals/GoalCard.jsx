...
export default function GoalCard({ goal, onUpdate, onDelete, onEdit }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className={`relative p-6 rounded-3xl border bg-surface backdrop-blur-xl transition-colors ${isCompleted ? 'border-success/30' : 'border-border'}`}
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-brand-light text-brand-dark">
              {goal.category}
            </span>
            <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-surface-3 text-text-3">
              {goal.type}
            </span>
            {isCompleted && (
              <span className="px-2.5 py-1 text-xs font-medium rounded-full bg-success/10 text-success flex items-center gap-1">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 rounded-full hover:bg-surface-3 text-text-3 transition-colors"
          >
          ...
          {showMenu && (
            <motion.div 
              ...
              className="absolute right-0 top-full mt-2 w-48 py-2 bg-surface border border-border rounded-2xl shadow-xl z-10 overflow-hidden"
            >
      ...
      <div className="space-y-3">
        ...
        <div className="h-2 w-full bg-border rounded-full overflow-hidden">
          <motion.div
            ...
            className={`h-full rounded-full ${isCompleted ? 'bg-success' : 'bg-brand'}`}
          />
        </div>
      </div>
